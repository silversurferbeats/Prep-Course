...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  let resultadoDeLaMultiplicacion = 1;
...
function mesesDelAño(array) {
  ...
  var arrayConLosMesesEcontrados = [];
  for(let i = 0; i < array.length; i++){
    if(array[i] === "Enero" || array[i] === "Marzo" || array[i] === "Noviembre"){
      arrayConLosMesesEcontrados.push(array[i]);
    }
  }
  if(arrayConLosMesesEcontrados.length === 3){
    return arrayConLosMesesEcontrados;
  }else{
...
function mayorACien(array) {
  ...
  // Tu código:
  let arrayConNumMayorACien = [];
  for(let i = 0; i < array.length; i++){
    if(array[i] > 100){
      arrayConNumMayorACien.push(array[i]);
    }
  }
  return arrayConNumMayorACien;
}
